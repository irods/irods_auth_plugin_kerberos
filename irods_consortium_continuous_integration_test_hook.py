        ...
        with open('/etc/hosts', 'r') as hosts_file:
            for l in hosts_file:
                ...
                if '127.0.0.1' in l and alias not in l:
                    hosts_copy.write('{} {}\n'.format(l.strip(), alias).encode('utf-8'))
                else:
                    hosts_copy.write(l.encode('utf-8'))
        hosts_copy.flush()
...
'''
    with tempfile.NamedTemporaryFile() as f:
        f.write(debconf_settings.encode('utf-8'))
        f.flush()
    ...
    def add_domain_to_krb5_conf():
        with tempfile.NamedTemporaryFile() as conf_copy:
            with open('/etc/krb5.conf', 'r') as conf:
                for l in conf:
                    conf_copy.write(l.encode('utf-8'))
...
'''
        with tempfile.NamedTemporaryFile() as conf_copy:
            with open('/etc/krb5.conf', 'r') as conf:
                for l in conf:
                    conf_copy.write(l.encode('utf-8'))
            conf_copy.write(conf_section.encode('utf-8'))
            conf_copy.flush()
            irods_python_ci_utilities.subprocess_get_output(['sudo', 'cp', conf_copy.name, '/etc/krb5.conf'], check_rc=True)
        ...
        with open('/etc/krb5.conf', 'r') as krb5_file:
             for l in krb5_file:
                 if 'default_ccache_name' in l:
                     conf_copy.write("#default_ccache_name = KEYRING:persistent:%{uid}".encode('utf-8'))
                 else:
                     conf_copy.write(l.encode('utf-8'))
        conf_copy.write(krb5_conf_contents.encode('utf-8'))
        conf_copy.flush()
        irods_python_ci_utilities.subprocess_get_output(['sudo', 'cp', conf_copy.name, '/etc/krb5.conf'], check_rc=True)
...
'''
    ...
    with tempfile.NamedTemporaryFile() as kdcconf_copy:
        with open('/var/kerberos/krb5kdc/kdc.conf', 'r') as kdc_file:
            for l in kdc_file:
                kdcconf_copy.write(l.encode('utf-8'))
        kdcconf_copy.write(kdc_conf_contents.encode('utf-8'))
        kdcconf_copy.flush()
    ...
    irods_python_ci_utilities.subprocess_get_output(['sudo', 'kdb5_util', 'create', '-r', 'EXAMPLE.ORG', '-s', '-W'],
                                                    data='{0}\n{0}\n'.format(kdc_database_master_key()).encode('utf-8'), check_rc=True)
...
krbtest
'''
    irods_python_ci_utilities.subprocess_get_output(['sudo','kadmin.local'], data=stdin.encode('utf-8'), check_rc=True)
...
def enable_admin_privileges_apt():
    ...
    with tempfile.NamedTemporaryFile() as kadm5_copy:
        with open('/etc/krb5kdc/kadm5.acl', 'r') as kadm5_file:
            for l in kadm5_file:
                kadm5_copy.write(l.encode('utf-8'))
...
'''.format(kdc_database_master_key(), principal, unprivileged_principal_password())
    irods_python_ci_utilities.subprocess_get_output(['sudo', 'kadmin', '-p', 'root/admin'], data=stdin.encode('utf-8'), check_rc=True)
...
def create_ticket_granting_ticket():
    irods_python_ci_utilities.subprocess_get_output(['sudo', 'kinit', 'krb_user'],
                                                    data='{0}\n'.format(unprivileged_principal_password()).encode('utf-8'),
                                                    check_rc=True)
    ...
    if options.do_setup:
        irods_python_ci_utilities.install_os_packages_from_files(
            glob.glob(os.path.join(os_specific_directory,
                      'irods-auth-plugin-krb*.{}'.format(package_suffix))
            )
        )
